...
const OrderStatusTracker = ({ historyStatus }) => {
  ...
  const hasCancelledStatus = historyStatus?.some(step => 
    ["CANCELLED", "RETURN_REQUESTED", "RETURN_APPROVED", "RETURNED", "REFUNDED"].includes(step.status)
  );
  ...
  const additionalStatuses = [
    ...
    { status: "RETURN_REQUESTED", id: "return-requested-default", updatedAt: new Date() },
    { status: "RETURN_APPROVED", id: "return-approved-default", updatedAt: new Date() },
  ...
  const mergedStatuses = statusesToShow.map(statusItem => {
    const realStatus = historyStatus?.find(s => s.status === statusItem.status);
  ...
  return (
    <>
      ...
      <div className="hidden md:flex items-center justify-between mb-3 pb-4 relative overflow-x-auto scrollbarWidthThin">
        <div className="flex  min-w-[600px]">
          {mergedStatuses.map((step, index) => {
            ...
            const isCompleted = index < mergedStatuses.findIndex(s => activeStatuses.includes(s.status)) || 
                                (index === mergedStatuses.findIndex(s => activeStatuses.includes(s.status)) && isActive);

            return (
              <div key={`desktop-${step.id}`} className="text-center flex-1 relative px-2">
                <div
                  className={`w-4 h-4 mx-auto rounded-full z-10 ${
                    isActive ? statusColor : isCompleted ? statusColor : "bg-gray-300"
                  }`}
                ></div>
                ...
                <div className={`text-xs mt-1 ${isActive ? "text-gray-800 font-medium" : "text-gray-500"} truncate`}>
                  {step.status.replace(/_/g, " ")}
                </div>
                <div className="text-xs text-gray-400 truncate">
                  {isActive ? format(new Date(step.updatedAt), "dd MMM yyyy, hh:mm a") : ""}
                </div>
              </div>
            );
          })}
          ...
          return (
            <div key={`mobile-${step.id}`} className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className={`text-sm ${isActive ? "text-gray-800 font-medium" : "text-gray-500"}`}>
                  {step.status.replace(/_/g, " ")}
                </div>
                {isActive && (
