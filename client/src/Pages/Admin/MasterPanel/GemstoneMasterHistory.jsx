...
import { getGemstoneVariantUpdateHistory } from "../../../api/Admin/GemstoneApi";

const GemstoneMasterHistory = () => {
  const { id } = useParams();
  ...
  const fetchHistoryData = async (page = 1) => {
    setLoading(true);
    try {
      const response = await getGemstoneVariantUpdateHistory(id, {
        page: filters.page || 1,
        limit: 10,
        search: debouncedSearch,
      });
      console.log(response.data.data.gemstoneHistory);

      console.log(response.data.data);

      setHistoryData(response.data.data.gemstoneHistory);
            ...
            {loading ? (
              ...
              historyData.map((item, idx) => (
                <TableRow key={item.id} className="text-center">
                  ...
                  <TableCell>{item.clarityLabel}</TableCell>
                  <TableCell>₹{item.gemstonePriceInCarat} carat</TableCell>
...
export default GemstoneMasterHistory;
